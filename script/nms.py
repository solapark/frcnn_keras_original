...
src_json_path = '/data3/sap/frcnn_keras/result/result-sv_messytable_cam3_resume_model_110/test_3cam/svdet+asnet+mvcnn.json'
dst_json_path = '/data3/sap/frcnn_keras/result/result-sv_messytable_cam3_resume_model_110/test_3cam/svdet+asnet+mvcnn+nms.json'
...
src_json = json_maker([], src_json_path, num_cam)
src_json.load()

dst_json = json_maker([], src_json_path, num_cam)
dst_json.load()
...
for scene_name in tqdm(scene_name_list) :
    ...
    bboxes = {cls_name : [ [] for _ in range(num_cam) ] for cls_name in inst_clss}
    is_valids = {cls_name : [ [] for _ in range(num_cam) ] for cls_name in inst_clss}
    ...
    for cls in bboxes:
        ...
        new_boxes_all_cam, new_probs, new_is_valids_all_cam = utility.non_max_suppression_fast_multi_cam(cur_bboxes, cur_probs, cur_is_valids, overlap_thresh=nms_thresh)
