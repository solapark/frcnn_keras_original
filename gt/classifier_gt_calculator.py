...
class CLASSIFIER_GT_CALCULATOR:
    ...
    def get_batch(self, *args):
        X_box_batch, Y_cls_batch, Y_regr_batch, iou_batch = [], [], [], []
        for args_one_batch in zip(*args) :
            X_box, Y_cls, Y_regr, iou = self.calc_classifier_gt(*args_one_batch)
            X_box_batch.append(X_box)
            Y_cls_batch.append(Y_cls)
            Y_regr_batch.append(Y_regr)
            iou_batch.append(iou)
        return np.array(X_box_batch), np.array(Y_cls_batch), np.array(Y_regr_batch), np.array(iou_batch)
    ...
    def get_gt_insts_box_cls(self, gt_insts):
        ...
        is_valid = np.zeros((num_inst, self.num_valid_cam))
        ...
        for pred_idx, (pred_boxes, is_pred_valid) in enumerate(zip(all_pred_boxes, is_pred_box_valid)):
            ...
            for gt_idx, (gt_boxes, is_gt_valid) in enumerate(zip(all_gt_boxes, is_gt_box_valid)):
                ...
                if cur_iou > best_iou : 
                    best_iou = cur_iou 
                    best_gt_idx = gt_idx
                    best_iou_list = [i/(u+1e-6) for i, u in zip(intersections, unions)]
                    
            if best_iou > self.max_overlap :
